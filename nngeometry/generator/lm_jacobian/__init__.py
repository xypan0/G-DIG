...
class Jacobian:
    ...
    def get_covariance_matrix(self, examples):
        ...
        for d in loader:
            ...
            bs = inputs.size(0)
            output = self.function(*d).view(bs, -1, self.n_output) \
                .sum(dim=0).sum(dim=0)
            for i in range(self.n_output):
                print(f'out: {output.shape}')
                print(f'inp: {inputs.shape}')
                self.grads.zero_()
    ...
    def get_kfac_blocks_o(self, examples):
        # add hooks
        self.handles += self._add_hooks(self._hook_savex,
        ...
        for d in loader:
            ...
            # print(d)
            # print(inputs)
            # print(inputs.size(0), self.n_output)
            # inputs.requires_grad = True
            ...
            if len(output.shape)==3:
                output = output.mean(dim=0).mean(dim=0)
            # elif len(output.shape)==2:
            #     output = output.view(bs, -1, self.n_output) \
            ...
            for self.i_output in range(self.n_output):
                ...
                # st=time.time()
                    # cur_inp=inputs[:, seq_idx, :]# .unsqueeze(1)
                    # print(f'cru_inp: {cur_inp.shape}')
                    # print(f'cru_oup: {output[seq_idx][self.i_output]}')

                # torch.autograd.grad(output[self.i_output], [inputs],
                                    # retain_graph=retain_graph)
                torch.autograd.backward(output[self.i_output], retain_graph=retain_graph)
                # tot=time.time()-st
                # print(f'time for one grad: {tot}s')
    ...
    def get_kfac_blocks(self, examples):
        ...
        for d in loader:
            ...
            loss=self.model(input_ids=inputs, labels=labels).loss
    ...
    def implicit_mv(self, v, examples):
        ...
        for d in loader:
            ...
            for i in range(self.n_output):
                ...
                torch.autograd.grad(f_output[:, i].sum(dim=0).sum(dim=0), [inputs],
                                    retain_graph=True,
                                    only_inputs=True)
    ...
    def implicit_vTMv(self, v, examples):
        ...
        for d in loader:
            ...
            f_output = self.function(*d).view(bs, self.n_output).sum(dim=0).sum(dim=0)
            for i in range(self.n_output):
                # TODO reuse instead of reallocating memory
                self._Jv = torch.zeros((1, bs), device=device)
    ...
    def _hook_compute_kfac_blocks(self, mod, gy):
        mod_class = mod.__class__.__name__
        # print(f'mod: {mod.__class__.__name__}')
        # print(f'm_to_l: {self.m_to_l}')
        # print(mod.weight.device)
        # print([k.weight.grad for k in self.m_to_l.keys()])  
        # print(f'test: {self.m_to_l[list(self.l_to_m.values())[0]]}')
        x = self.xs[mod]
        # try: 
        layer_id = self.m_to_l[mod]
        # except KeyError:
            # print(mod.weight.device)
            # print([k.weight.device for k in self.m_to_l.keys()]) 
            # return
        # print(f'x: {x}')
        # print(f'gy: {gy}')
        # print(f'x: {x.shape}')
        ...
        if mod_class in ['Linear', 'Conv2d']:
            ...
            # gy=gy.mean(1)
            # x=x.mean(1)
            gy=gy.sum(0).sum(0).unsqueeze(0)
            x=x.sum(0).sum(0).unsqueeze(0)
            # print(f'gy: {gy.sum(1)}')
