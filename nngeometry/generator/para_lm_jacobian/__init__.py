...
class Jacobian:
    ...
    def get_kfac_blocks(self, examples):
        ...
        self._blocks = dict()
        for layer_id, layer in self.layer_collection.layers.items():
            self.l_to_m[layer_id].weight.retain_grad()
        ...
        for d in loader:
            ...
            # print(torch.cuda.memory_allocated(device))
            # print(f'layers: {list(self.model.named_modules())}')
            for self.i_output in range(self.n_output):
                ...
                for layer_id in self.layer_collection.layers.keys():
                    print(f'grad: {self.l_to_m[layer_id]}')
                    print(f'grad: {self.l_to_m[layer_id].weight.grad}')
                # tot=time.time()-st
